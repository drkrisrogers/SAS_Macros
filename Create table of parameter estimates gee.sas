%macro est_gee(table=work.estimates,output=genmod);
%local levels;
%if (&output=genmod) %then %do;
	data work.estimates (keep=parameter level1 ci where=(parameter ne 'Scale'));
		set work.estimates (rename=(Parm=parameter));
